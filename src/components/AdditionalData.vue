    ...
    <div v-if="currentData" class="h-100 w-100 p-3 d-flex flex-column justify-content-between">
        ...
        <div>
            <div class="input-group rounded w-100">
                <input v-on:keyup.enter="search()" v-model="city" type="search" class="form-control rounded" placeholder="Enter City Name..." aria-label="Search" aria-describedby="search" />
                <span class="input-group-text border-0 text-warning" id="search" v-on:click="search()">
        ...
        <div>
            ...
            <div v-if="dailyData" class="row">
                <WeekChart ref="weekChart" :data="dailyData" ></WeekChart>
            </div>
...
export default {
    ...
    methods: {
        async search() {
            if(this.city != null) {
                this.$router.push('/weather?city=' + this.city)
                await this.$emit('changeCity', this.city) // send trigger to parent component
                this.$refs.weekChart.initChart() // call child class
            }
        },
        getTime(unix) {
